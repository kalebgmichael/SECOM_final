...
public class EncryptionPubController {
    ...
    public String getenc_sig_peer(@RequestParam("message") String message, @RequestParam("publickey_peer") String publickey_peer) throws Exception {
        ...
        System.out.println("getenc_sig_peer:this is the sym_key used to encrypt the DH publickey"+sym_key);
        String encryptedPubKey= encryptionPubService.encrypt_sym(message1);
        System.out.println("getenc_sig_peer:the symmetrically encryptedPubKey to be sent"+ encryptedPubKey);
        System.out.println("getenc_sig_peer:the encryptedPubKey to be sent length"+ message.length());
//        try
//        {
            String encryptedMessage= encryptionPubService.encrypt_peer(sym_key);
            System.out.println("getenc_sig_peer:This is the 9th message encrypted symkey with public key of the peer to be decrypted by private key of the pper"+encryptedMessage);
            encryptionPubService.initFromStringsprivate();
